...
def test_data(input_test, output_test, lengths, h_state, model):
    ...
    nllloss = nn.NLLLoss()
    loss = nllloss(out, output_test)
    print('test: ', loss)
    return loss

def caculate_acc(outputY, valY):
    outY = torch.argmax(outputY, dim = 1)
    return (outY == valY).sum()/outputY.size()[0]
...
def test(path):
    ...
    Data_train, Data_test = baseone.Datasplit(baseone.datas, count, 0)
    Data_train = baseone.inputs2T(Data_train)
    Data_test = baseone.inputs2T(Data_test)
    ...
    train_Iter(Data_train, state, rnn, optimer, 10, 90)
    test_data(Data_test[0], Data_test[1], Data_test[2], state, rnn)
